...
UIEvents::newFrame() ‚Ü?Renderer::DrawFrame()
                                ...
                                ‚îú‚îÄ‚îÄ Draw (3 vertices for triangle)
                                ‚îú‚îÄ‚îÄ End dynamic rendering
                                ‚îî‚îÄ‚îÄ Swapchain::Present()
```
...
- **Sampling**: Hemisphere samples in view-space, random rotation via 16√ó16 noise texture
...
- **Edge detection**: 3√ó3 Laplacian-style kernel over neighboring IDBuffer pixels.
...
- **Push constant**: `SunShadowPushConstants` carries `mat4 lightViewProj` (view √ó projection)
- **Shadow map resolution**: 2048√ó2048, `VK_FORMAT_D32_SFLOAT`
...
| ShadowMap | D32_SFLOAT | 1.0 | Per-light shadow depth (RenderCache-owned). Cubemap (6-layer 2D_ARRAY, 1024√ó1024) for point lights; 2D (2048√ó2048) for sun lights |
