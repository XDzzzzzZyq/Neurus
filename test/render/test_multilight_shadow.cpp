 ...
 * the full deferred pipeline (ShadowDepthPass 鈫?GeometryPass 鈫?ShadowIntensityPass
 * 鈫?LightingPass) and validates HDRColor output via reference image regression.
 *
 * Reference image pattern:
 *   - First run: generates reference PNG 鈫?GTEST_SKIP
 *   - Second run: compares pixel-by-pixel with 卤2 tolerance 鈫?PASS/FAIL
 *
 * This is the RED phase of TDD 鈥?the test compiles and runs but the shadow
...
protected:
	...
	void SetUp() override
	{
		...
		// --- Upload manager for CPU鈫扜PU struct conversion ---
	}
...
// ===========================================================================
// TwoShadowLights_HDRColorReference 鈥?reference image regression
// ===========================================================================
...
TEST_F(MultiLightShadowTest, TwoShadowLights_HDRColorReference)
{
	...
	ctx.frameIndex   = 0;
	// renderItems removed 鈥?ShadowDepthPass iterates scene.mesh_list directly
	...
	//         Order: ShadowDepth (independent) 鈫?Geometry (writes G-Buffer)
	//                鈫?ShadowIntensity (reads Position + cubemap)
	//                鈫?Lighting (reads G-Buffer + shadow intensity)
	// -------------------------------------------------------------------
	{
		auto& cmd = BeginCmd();
	...
	if (refResult < 0)
	{
		// First run 鈥?reference generated (return -1) or load failure (-2)
		if (refResult == -1)
	...
	{
		// Second run 鈥?compare against reference
		EXPECT_EQ(refResult, 0) << refResult << " pixel(s) differ from reference (tol=卤2)";
	}
}

// ===========================================================================
// TwoLights_NoVUID 鈥?smoke test (pipeline runs without crash)
// ===========================================================================
...
// ===========================================================================
// ShadowIntensityReadback 鈥?verify per-light shadow intensity data
// ===========================================================================
 ...
 * contain both shadowed (==1.0) and lit (==0.0) pixels 鈥?an all-zero
...
TEST_F(MultiLightShadowTest, ShadowIntensityReadback_VerifyNonZero)
{
	...
	// -------------------------------------------------------------------
	// Run ONLY ShadowDepth 鈫?Geometry 鈫?ShadowIntensity (NO LightingPass)
	...
	// -------------------------------------------------------------------
	{
		...
		m_geometryPass->Record(*cmd, *m_renderCache, ctx);
		m_shadowIntensityPass->Record(*cmd, *m_renderCache, ctx);
		// NO LightingPass here 鈥?pure isolation test
		EndSubmitWait(cmd);
	}
	...
	for (int lightUID : shadowRes.lightUIDs)
	{
		...
		EXPECT_GT(shadowed, 0)
			<< "Light " << lightUID << " shadow intensity is ALL ZERO 鈥?"
			<< "shadow evaluation pass may be broken (G-Buffer Position, cubemap depth, or dispatch)";
		EXPECT_GT(lit, 0)
			<< "Light " << lightUID << " shadow intensity is ALL ONE 鈥?"
...
// ===========================================================================
// ShadowIntensityPerLight_ReferenceImage 鈥?per-light shadow intensity regression
// ===========================================================================
 ...
 *
 * Runs the full deferred+PBR pipeline (ShadowDepth 鈫?Geometry 鈫?
 * ShadowIntensity 鈫?Lighting) with 2 lights, then reads back each
...
TEST_F(MultiLightShadowTest, ShadowIntensityPerLight_ReferenceImage)
{
	...
	// -------------------------------------------------------------------
	// Run full pipeline (ShadowDepth 鈫?Geometry 鈫?ShadowIntensity 鈫?Lighting)
	// -------------------------------------------------------------------
	{
		auto& cmd = BeginCmd();
		m_shadowDepthPass->Record(*cmd, *m_renderCache, ctx);
	...
	for (size_t li = 0; li < shadowRes.lightUIDs.size(); ++li)
	{
		...
		if (refResult < 0)
		{
			// First run 鈥?reference generated
			if (refResult == -1)
				anyGenerated = true;
			else
			{
		...
		else if (refResult > 0)
		{
			...
			ADD_FAILURE() << refResult
				<< " pixel(s) differ from reference for light " << li << " (tol=卤2)";
		}
...
// ===========================================================================
// SunLights_HDRColorReference 鈥?full deferred pipeline with sun shadows
// ===========================================================================
 ...
 * (ShadowDepth 鈫?Geometry 鈫?ShadowIntensity 鈫?Lighting) runs and the final
